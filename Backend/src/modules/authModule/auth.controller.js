...
export const register = async (req, res, next) => {
  ...
  if (isExist)
    return next(
      new Error("Email already exist", { cause: StatusCodes.BAD_REQUEST })
    );
  ...
  const { accessToken, refreshToken } = await createToken(user.role, {
    id: user._id,
  });
...
export const logIn = async (req, res, next) => {
  ...
  if (!compare(password, user.password))
    return next(
      new Error("in-correct email or password", {
...
export const refreshToken = async (req, res, next) => {
  ...
  const { user, accessSignature } = await decodeToken(
    refreshToken,
    tokenTypes.refresh,
    next
  );
